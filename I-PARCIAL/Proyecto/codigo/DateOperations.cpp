...
bool DateOperations::checkDate(Date date){
    ...
    if(validateDate(date.getDay(),date.getMonth(),date.getYear())){
        ...
        if(day == 0 || day == 6){
            ...
            flag = validarIngresoFeriado(date.getDay(),date.getMonth(),date.getYear());
        }
    }else{
        flag = false;
    }
...
Date* DateOperations::generateDate(int numbermonth,Date date){
    Date *dates = new Date[numbermonth];
...
bool DateOperations::validarIngresoFeriado(int day,int month,int year){
